...
$db=new packageServicesSelectedDAL(include("../../../dbConfig.php"));
switch ($request_method) {
        case 'GET':
			...
			else if(!empty($_GET["packageId"])){
				try {
                    header('Content-Type: application/json');
					$results = $db->LoadBypackageId($_GET["packageId"])->fetchAll(PDO::FETCH_ASSOC);
					if(isset($results[0])){
        ...
        case 'POST':
			try {
				...
				$portfolioCategory=new packageServicesSelectedBAL();
				$portfolioCategory->packageId=$data["packageId"];
				$portfolioCategory->packegeServiceId=$data["packegeServiceId"];
				header('Content-Type: application/json');
