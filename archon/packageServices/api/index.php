...
$db=new packageServicesDAL(include("../../../dbConfig.php"));
switch ($request_method) {
        ...
        case 'POST':
			try {
				...
				$packageService=new packageServicesBAL();
				$packageService->title=$data["title"];
				header('Content-Type: application/json');
        ...
        case 'PUT':
			try {
				...
				$id=intval($_GET["id"]);
				$packageService=new packageServicesBAL();
				$packageService->id=$data["id"];
				$packageService->title=$data["title"];
				header('Content-Type: application/json');
				$db->Update($packageService);
